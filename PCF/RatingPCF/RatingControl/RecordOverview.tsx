...
export const RecordOverview: React.FC<RecordOverviewProps> = (overviewProps: RecordOverviewProps) => {
    ...
    const retrieveData = async (context: ComponentFramework.Context<IInputs>, id: string) => {
        ...
        if (context.userSettings.userName==="") {//local enviromment 
            return [
               { EntityCollectionName: "Activities", TotalCount: 10 },
                {EntityCollectionName: "Case", TotalCount: 5 },
            { EntityCollectionName: "Opportunity", TotalCount: 3 }]
        }
